...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  Class.prototype.config = {
    ...
    , tr_click_checkable: false // tr click 事件时 是否选中 本行 再次点击取消选中 add by roc 
  };
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){
      ...
      layui.each(data, function(i1, item1){
        var tds = [], tds_fixed = [], tds_fixed_r = [], tr_attributes = []
        ...
        that.eachCols(function(i3, item3){
            ...
            ,'<div class="layui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
              ...
              if(item3.type === 'checkbox'){
                return '<input type="checkbox" name="layTableCheckbox" lay-skin="primary" '+ function(){
                  ...
                  if (item3[checkName]) {
                      var chkFlag = false;
                      if ($.isFunction(item3[checkName])) {
                          chkFlag = item3[checkName](content, item1, i1);
                      }
                      //item1[checkName] = item3[checkName];
                      //return item3[checkName] ? 'checked' : '';
                      item1[checkName] = chkFlag;
                      return chkFlag ? 'checked' : '';
                  }
                    //add by roc end
              ...
              if(item3.toolbar){
                ...
                //add by roc begin
              if (item3.templet) content = laytpl($(item3.templet).html() || String(content)).render(tplData);
              if (item3.content) content = item3.content(content, item1, i1);
              return content;
                //add by roc end
         ...
         //add by roc begin
        tr_attributes = calculateObjectValue(options, options.rowAttributes, [item1, i1], {});
        var htmlAttributes = [];
        if (tr_attributes) {
            for (var key in tr_attributes) {
                htmlAttributes.push(sprintf('%s="%s"', key, escapeHTML(tr_attributes[key])));
            }
        }
        trs.push('<tr', sprintf(' %s', htmlAttributes.join(' ')), ' data-index="' + i1 + '">' + tds.join('') + '</tr>');
        //add by roc end
  ...
  //add by roc 获得选择数据
  Class.prototype.getCheckData = function (index) {
      var that = this
     , config = that.config
     , thisData = table.cache[that.key];
      if (!thisData[index]) return false;
      return thisData[index][config.checkName];
  };
  ...
  Class.prototype.events = function(){
    ...
    that.layBody.on('mouseenter', 'tr', function(){
      ...
      //add by roc 实现 单击tr 选择checkbox
    that.layBody.on('click', 'tr', function () {
        var tr = $(this), index = tr.index();

        if (config.tr_click_checkable) {
            var chk = tr.find('input[name="layTableCheckbox"]');
            var checked = that.getCheckData(index);
        ...
        layui.event.call(this, MOD_NAME, 'tr_click(' + filter + ')', { tr: tr, index: index });
    });
    //add by roc end
